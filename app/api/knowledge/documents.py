...
def create_document(
    ...
    if payload.file_path and Path(payload.file_path).exists():
        try:
            content_hash = _compute_file_hash(payload.file_path)
...
def delete_document(
    ...
    try:
        delete_document_vectors(current_user.tenant_id, document_id)
    except Exception as e:
...
@router.post("/{document_id}/chunks", response_model=DocumentChunksCreateResponse, status_code=status.HTTP_201_CREATED, tags=["Chunks"])
