    ...
    async def refresh(self) -> None:
        ...
        try:
            ...
            if response.status_code != 200:
                raise AuthError(
                    f"Google Drive OAuth refresh failed ({response.status_code}): {response.text}"
                )
            ...
            logger.info("Google Drive OAuth2 access token refreshed successfully.")
        except Exception as e:
            if isinstance(e, AuthError):
                raise
            raise AuthError(f"Exception during Google Drive token refresh: {str(e)}") from e
