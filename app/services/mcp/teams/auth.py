"""MS Graph Teams OAuth2 authentication handler."""
import time
...
class TeamsAuth:
    """Manages Microsoft Graph API credentials and token refreshing using HTTPX."""
    ...
    async def refresh(self) -> None:
        ...
        logger.info("Refreshing MS Graph Teams access token...")
        ...
        try:
            ...
            if response.status_code != 200:
                raise AuthError(
                    f"MS Graph Teams OAuth refresh failed ({response.status_code}): {response.text}"
                )
            ...
            logger.info("MS Graph Teams access token refreshed successfully.")
        except Exception as e:
            if isinstance(e, AuthError):
                raise
            raise AuthError(f"Exception during MS Graph Teams token refresh: {str(e)}") from e
